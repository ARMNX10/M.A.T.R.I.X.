...
async def send_message(message, box_id):
    if clients:
        await asyncio.wait([client.send(json.dumps({"message": message, "box_id": box_id})) for client in clients])
...
def takecommand():
    ...
    try:
        ...
        print(f"\n==> Aarav: {query}")
        asyncio.run(send_message(query, "Aarav"))
        return query.lower()
    except:
        return ""
...
def play_random_music():
    ...
    try:
        # This might open your default media player or application associated with the file extension
    ...
    try:
        async for message in websocket:
            ...
            if data["action"] == "start":
                await asyncio.to_thread(wakeup)
    finally:
        await unregister(websocket)

start_server = websockets.serve(handler, "localhost", 8765)

async def wakeup_and_start_server():
    loop = asyncio.get_event_loop()
    await start_server
    wakeup()  # Call the wakeup function here
    loop.run_forever()

if __name__ == '__main__':
    asyncio.run(wakeup_and_start_server())